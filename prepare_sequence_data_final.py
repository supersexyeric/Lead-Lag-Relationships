...
# --- Configuration ---
INPUT_FILE = "/home/ubuntu/full_features_crypto_data_unscaled.csv" # Data with raw features and indicators
OUTPUT_DIR = "/home/ubuntu/"
SCALER_PATH = os.path.join(OUTPUT_DIR, "scaler.joblib")
FEATURE_LIST_PATH = os.path.join(OUTPUT_DIR, "feature_columns.npy")
SEQUENCE_DATA_PATH = os.path.join(OUTPUT_DIR, "sequence_data.npz")

# Parameters (Adjusted due to memory constraints, original report might differ)
SEQUENCE_LENGTH = 15 # Original might be larger, e.g., 60
PREDICTION_HORIZON = 5
TRAIN_SPLIT_RATIO = 0.8
TARGET_ASSET_PREFIX = "btc" # Asset to predict
...
# Define target variable (future return)
# Using btc_close as the target, shifted by the prediction horizon
df["target"] = df[f"{TARGET_ASSET_PREFIX}_close"].shift(-PREDICTION_HORIZON)

# Define features (use all calculated indicators and basic features for all assets)
# Exclude raw OHLCV columns and the target column itself before shifting
exclude_cols = [col for col in df.columns if any(x in col for x in ["_open", "_high", "_low", "_volume"]) and not col.endswith("_OBV")]
exclude_cols.append("target") # Exclude the target variable itself
# Also exclude the non-scaled close prices used for calculating indicators/target
exclude_cols.extend([f"{asset}_close" for asset in ["btc", "eth", "sol"]])

feature_columns = [col for col in df.columns if col not in exclude_cols]
print(f"Selected {len(feature_columns)} features:")
# print(feature_columns) # Uncomment to see the full list
...
def create_sequences(X, y, seq_len, horizon):
    ...
    for i in range(len(X) - seq_len - horizon + 1):
        ...
        ys.append(y[i + seq_len + horizon - 1]) # Target is PREDICTION_HORIZON steps ahead of the *end* of the sequence
